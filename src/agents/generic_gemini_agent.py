...
class GenericGeminiAgent:
    def __init__(self, config: AgentConfig, api_key: str | None = None):
        ...
        if config.provider in {"gemini", "gemini_api"}:
            if not api_key:
                raise ValueError("api_key is required for gemini_api provider")
    ...
    def _build_cli_prompt(system_prompt: str, user_message: str) -> str:
        return (
            f"{system_prompt}\n\n"
            "아래 사용자 메시지를 분석하세요.\n"
            "반드시 AgentResponse JSON 스키마 형식의 '순수 JSON 객체만' 출력하세요.\n"
            "마크다운, 설명 문장, 코드블록, 추가 텍스트를 절대 포함하지 마세요.\n\n"
            f"사용자 메시지:\n{user_message}"
        )
    ...
    def run(self, user_message: str) -> AgentResponse:
        ...
        if self.config.provider in {"gemini", "gemini_api"}:
