...
NOW = datetime.fromtimestamp(1780536388000 / 1000, tz=timezone.utc)
...
class FakeHttpClient:
    ...
    def get_json(self, base_url: str, path: str, params: dict | None = None) -> HttpJsonResponse:
        ...
        if path == self.fail_path:
            ...
            setattr(error, "safe_response_preview", '{"retCode":10000,"retMsg":"temporary public failure"}')
            setattr(error, "exchange_error_code", 10000)
...
def _bybit_responses(*, watch_gap: bool = False, missing_ticker: bool = False, retcode_error: bool = False) -> dict[str, dict | None]:
    fixture = json.loads((FIXTURE_DIR / "bybit_valid_btcusdt_linear.json").read_text(encoding="utf-8"))
    if watch_gap:
        fixture["ticker_response"]["result"]["list"][0]["markPrice"] = "100.00"
        fixture["ticker_response"]["result"]["list"][0]["indexPrice"] = "100.00"
        fixture["orderbook_response"]["result"]["b"] = [["98.80", "10.000"]]
        fixture["orderbook_response"]["result"]["a"] = [["99.00", "5.000"]]
    if retcode_error:
        fixture["ticker_response"] = {"retCode": 10001, "retMsg": "public symbol invalid", "result": {}, "time": 1780536387150}
    return {
        BybitMarkOrderbookGapHuntAdapter.TICKER_PATH: None if missing_ticker else fixture["ticker_response"],
...
def _adapter(fake: FakeHttpClient, **config: object) -> BybitMarkOrderbookGapHuntAdapter:
    merged_config = {
    ...
    return BybitMarkOrderbookGapHuntAdapter(
        http_client=fake,
        now_fn=lambda: NOW,
...
class BybitMarkOrderbookGapHuntAdapterTests(unittest.TestCase):
    def test_successful_public_bundle_creates_analysis_only_packet(self) -> None:
        fake = FakeHttpClient(_bybit_responses())
        ...
        self.assertEqual(packet.observations[0].venue_name, "Bybit Derivatives V5")
        self.assertEqual(packet.observations[0].market_symbol, "BTCUSDT")
        self.assertEqual(packet.observations[0].instrument_type, "linear_perpetual")
        ...
        self.assertEqual(metadata["adapter_id"], "live_bybit_mark_orderbook_gap_btcusdt")
        self.assertEqual(metadata["adapter_type"], "bybit_mark_orderbook_gap_hunt")
    ...
    def test_parser_output_ok_feeds_readiness_helper(self) -> None:
        packet = _adapter(FakeHttpClient(_bybit_responses())).fetch_packet()

        parser_output = packet.extensions["parser_output"]
        readiness = packet.extensions["readiness"]
        self.assertEqual(parser_output["parser_mode"], "bybit_linear")
    ...
    def test_watch_remains_analysis_only_without_execution_council_or_alert(self) -> None:
        packet = _adapter(FakeHttpClient(_bybit_responses(watch_gap=True))).fetch_packet()
    ...
    def test_missing_ticker_response_produces_need_data_candidate(self) -> None:
        packet = _adapter(FakeHttpClient(_bybit_responses(missing_ticker=True))).fetch_packet()
        ...
        self.assertIn("ticker_response", packet.candidates[0].required_missing_fields)
        self.assertEqual(len(packet.extensions["diagnostics"]), 3)

    def test_bybit_retcode_failure_raises_adapter_error_with_safe_diagnostics(self) -> None:
        fake = FakeHttpClient(_bybit_responses(retcode_error=True))
        ...
        self.assertEqual(diagnostics[0]["endpoint"], BybitMarkOrderbookGapHuntAdapter.TICKER_PATH)
        self.assertEqual(diagnostics[0]["params"], {"category": "linear", "symbol": "BTCUSDT"})
        self.assertEqual(diagnostics[0]["retCode"], 10001)
        self.assertEqual(diagnostics[0]["retMsg"], "public symbol invalid")
        self.assertIn("public symbol invalid", diagnostics[0]["safe_response_preview"])
    ...
    def test_public_http_failure_raises_adapter_error_with_safe_diagnostics(self) -> None:
        fake = FakeHttpClient(_bybit_responses(), fail_path=BybitMarkOrderbookGapHuntAdapter.TICKER_PATH)
        with self.assertRaises(MarketDataAdapterError) as ctx:
            _adapter(fake).fetch_packet()
    ...
    def test_adapter_uses_only_allowed_public_bybit_market_paths(self) -> None:
        fake = FakeHttpClient(_bybit_responses())
        _adapter(fake).fetch_packet()
        ...
        self.assertEqual(
            paths,
            [
                BybitMarkOrderbookGapHuntAdapter.TICKER_PATH,
        ...
        for _, path, params in fake.calls:
            self.assertTrue(path.startswith("/v5/market/"))
            self.assertNotIn("/private", path.lower())
            self.assertNotIn("/v5/order", path.lower())
            self.assertNotIn("/v5/account", path.lower())
            self.assertNotIn("/v5/position", path.lower())
            self.assertNotIn("/v5/asset/transfer", path.lower())
