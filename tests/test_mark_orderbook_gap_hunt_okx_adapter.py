...
class FakeHttpClient:
    def __init__(self, responses: dict[str, dict | None] | None = None, *, fail_path: str | None = None) -> None:
    ...
    def get_json(self, base_url: str, path: str, params: dict | None = None) -> HttpJsonResponse:
        ...
        if path == self.fail_path:
            ...
            setattr(error, "safe_response_preview", '{"code":"51000","msg":"temporary public failure"}')
            setattr(error, "exchange_error_code", "51000")
            setattr(error, "exchange_error_message", "temporary public failure")
            raise error
...
def _okx_responses(
    *,
    ...
    missing_mark: bool = False,
    missing_books: bool = False,
    code_error: bool = False,
) -> dict[str, dict | None]:
    fixture = json.loads((FIXTURE_DIR / "okx_valid_btc_usdt_swap.json").read_text(encoding="utf-8"))
    if watch_gap:
        fixture["mark_response"]["data"][0]["markPx"] = "100.00"
        fixture["books_response"]["data"][0]["bids"] = [["98.80", "10.000", "0", "1"]]
        fixture["books_response"]["data"][0]["asks"] = [["99.00", "5.000", "0", "1"]]
    if missing_books:
        fixture["books_response"]["data"][0]["bids"] = []
        fixture["books_response"]["data"][0]["asks"] = []
    if code_error:
        fixture["mark_response"] = {"code": "51000", "msg": "public instrument invalid", "data": []}
    return {
        OkxMarkOrderbookGapHuntAdapter.MARK_PRICE_PATH: None if missing_mark else fixture["mark_response"],
...
def _adapter(fake: FakeHttpClient, **config: object) -> OkxMarkOrderbookGapHuntAdapter:
    merged_config = {
    ...
    return OkxMarkOrderbookGapHuntAdapter(
        http_client=fake,
        now_fn=lambda: NOW,
...
class OkxMarkOrderbookGapHuntAdapterTests(unittest.TestCase):
    def test_successful_public_bundle_creates_analysis_only_packet(self) -> None:
        fake = FakeHttpClient(_okx_responses())
        ...
        self.assertEqual(packet.observations[0].venue_name, "OKX")
        self.assertEqual(packet.observations[0].market_symbol, "BTC-USDT-SWAP")
        self.assertEqual(packet.observations[0].instrument_type, "linear_swap")
        ...
        self.assertEqual(metadata["adapter_id"], "live_okx_mark_orderbook_gap_btc_usdt_swap")
        self.assertEqual(metadata["adapter_type"], "okx_mark_orderbook_gap_hunt")
    ...
    def test_fetch_snapshot_returns_packet_json_dict(self) -> None:
        snapshot = _adapter(FakeHttpClient(_okx_responses())).fetch_snapshot()
    ...
    def test_parser_output_okx_swap_feeds_readiness_helper(self) -> None:
        packet = _adapter(FakeHttpClient(_okx_responses())).fetch_packet()

        parser_output = packet.extensions["parser_output"]
        readiness = packet.extensions["readiness"]
        self.assertEqual(parser_output["parser_mode"], "okx_swap")
    ...
    def test_watch_remains_analysis_only_without_execution_council_or_alert(self) -> None:
        packet = _adapter(FakeHttpClient(_okx_responses(watch_gap=True))).fetch_packet()
    ...
    def test_missing_mark_response_produces_need_data_candidate(self) -> None:
        packet = _adapter(FakeHttpClient(_okx_responses(missing_mark=True))).fetch_packet()

        self.assertEqual(packet.extensions["parser_output"]["normalized_status"], "NEED_DATA")
        self.assertEqual(packet.candidates[0].metrics["readiness_status"], "NEED_DATA")
        self.assertIn("mark_response", packet.candidates[0].required_missing_fields)
        self.assertEqual(len(packet.extensions["diagnostics"]), 3)

    def test_missing_books_bids_asks_produces_need_data_candidate(self) -> None:
        packet = _adapter(FakeHttpClient(_okx_responses(missing_books=True))).fetch_packet()

        self.assertEqual(packet.extensions["parser_output"]["normalized_status"], "NEED_DATA")
        self.assertEqual(packet.candidates[0].metrics["readiness_status"], "NEED_DATA")
        self.assertIn("bid", packet.candidates[0].required_missing_fields)
        self.assertIn("ask", packet.candidates[0].required_missing_fields)

    def test_okx_code_failure_raises_adapter_error_with_safe_diagnostics(self) -> None:
        fake = FakeHttpClient(_okx_responses(code_error=True))
        ...
        self.assertEqual(diagnostics[0]["endpoint"], OkxMarkOrderbookGapHuntAdapter.MARK_PRICE_PATH)
        self.assertEqual(diagnostics[0]["params"], {"instType": "SWAP", "instId": "BTC-USDT-SWAP"})
        self.assertEqual(diagnostics[0]["code"], "51000")
        self.assertEqual(diagnostics[0]["msg"], "public instrument invalid")
        self.assertIn("public instrument invalid", diagnostics[0]["safe_response_preview"])
    ...
    def test_public_http_failure_raises_adapter_error_with_safe_diagnostics(self) -> None:
        fake = FakeHttpClient(_okx_responses(), fail_path=OkxMarkOrderbookGapHuntAdapter.MARK_PRICE_PATH)
        with self.assertRaises(MarketDataAdapterError) as ctx:
            _adapter(fake).fetch_packet()
    ...
    def test_adapter_uses_only_allowed_public_okx_paths(self) -> None:
        fake = FakeHttpClient(_okx_responses())
        _adapter(fake).fetch_packet()
        ...
        for _, path, params in fake.calls:
            self.assertTrue(path.startswith("/api/v5/public/") or path == "/api/v5/market/books")
            self.assertNotIn("/private", path.lower())
            self.assertNotIn("/account", path.lower())
            self.assertNotIn("/trade/order", path.lower())
            self.assertNotIn("/asset/transfer", path.lower())
