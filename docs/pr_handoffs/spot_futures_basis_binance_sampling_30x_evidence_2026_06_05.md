# Spot-Futures Basis Binance 30-Sample Extended Sampling Evidence v0

## 1. 작업 목적

이 문서는 `spot_futures_basis_v0`의 첫 user-local 30-sample extended sampling result를 기록하는 docs-only evidence handoff이다.

목적:

- `spot_futures_basis_v0`의 첫 user-local 30-sample extended sampling result를 기록한다.
- 이번 evidence는 `sample_market_data` path가 30 consecutive public-read-only samples에서 `market_sampling_v1` summary 생성까지 성공했음을 의미한다.
...
```bash
python tools/sample_market_data.py --adapter live_binance_spot_futures_basis_btcusdt --samples 30 --interval 2 --output data/market_samples/spot_futures_basis_binance_sampling_30x_summary.json
```
...
- User must delete `data/market_samples/spot_futures_basis_binance_sampling_30x_summary.json` after inspection.
...
| --- | --- | --- | --- | ---: | --- | --- | ---: | --- | --- | ---: | --- | --- | --- |
| First sample | `ok` | `spot_futures_basis` | `spot_futures_basis_v0` | 1 | `REJECT` | `REJECT` | `-0.13967667333565484` | `[]` | `OK` | 7 | `true` | `NO_TRADE_ONLY` | `spot_futures_basis_observation` |
| Last sample | `ok` | `spot_futures_basis` | `spot_futures_basis_v0` | 1 | `REJECT` | `REJECT` | `-0.15613142446464676` | `[]` | `OK` | 7 | `true` | `NO_TRADE_ONLY` | `spot_futures_basis_observation` |

## 5. REJECT / NO_PERSISTENT_EDGE interpretation

- `REJECT`는 sampling failure가 아니다.
- `NO_PERSISTENT_EDGE`는 adapter failure가 아니다.
- `samples_ok=30`, `samples_error=0`이므로 extended sampling path는 성공했다.
...
- `data/market_samples/spot_futures_basis_binance_sampling_30x_summary.json`는 user-local smoke artifact이며 commit 금지다.
...
1. Bybit public source research against common contract.
2. OKX public source research against common contract.
3. Spot-Futures Basis Binance baseline comparative summary.
