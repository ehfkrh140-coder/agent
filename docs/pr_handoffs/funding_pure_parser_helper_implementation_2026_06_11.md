...
- Branch name: `codex/funding-pure-parser-helper-implementation-pr188-fix`.
- Base branch: local replacement branch created from `2644a20` after verifying the previous checkout contained PR #187 planning docs; no `origin` remote is configured in this Codex workspace, so fallback freshness checks were used. The replacement branch intentionally keeps only the implementation files so GitHub PR Files changed can match the expected 3-file scope when opened against main that already contains PR #187.
...
- Commit parent hash: replacement implementation commit parent is `2644a20`; fallback check confirmed the previously verified checkout parent was not blocked parent `456b985`.
...
## PR #188 Merge Hold Correction

PR #188 was merge-held because GitHub PR Files changed showed 5 files while Expected Files Changed was 3 files. The extra files were `docs/funding_rate_pure_parser_helper_plan.md` and `docs/pr_handoffs/funding_pure_parser_helper_plan_2026_06_11.md`, which belong to PR #187 and must not reappear in the implementation PR diff. This replacement branch keeps only the implementation helper, its tests, and this implementation handoff. Codex local staged files are not the source of truth; GitHub PR Files changed must be checked by the reviewer and must match the 3 expected files exactly before merge consideration.
...
Actual staged files from `git diff --cached --name-only` on the replacement branch:

- `docs/pr_handoffs/funding_pure_parser_helper_implementation_2026_06_11.md`
- `src/market_data/funding_rate_parser.py`
- `tests/test_funding_rate_parser.py`

Comparison result: Same three files, no extra staged files. The two PR #187 planning docs are not staged in this replacement branch. Reviewers must still confirm GitHub PR Files changed matches Expected Files Changed exactly; if not, merge is on hold.
...
- `git status --short --branch`: passed; pre-edit status was clean in the verified checkout, and staged status later showed only expected changes on `codex/funding-pure-parser-helper-implementation-pr188-fix`.
...
- `python -m unittest tests.test_funding_rate_parser`: passed; output ended with `Ran 24 tests in 0.032s` and `OK`.
- `python -m unittest discover -s tests`: passed; output ended with `Ran 555 tests in 16.706s` and `OK`.
