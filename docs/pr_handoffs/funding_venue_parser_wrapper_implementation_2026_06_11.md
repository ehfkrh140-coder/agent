...
## PR #192 Merge Hold Replacement Evidence

- PR #192 was placed on merge hold because GitHub Files changed did not match Expected Files Changed.
- Expected Files Changed for the wrapper implementation were exactly `src/market_data/funding_rate_wrappers.py`, `tests/test_funding_rate_wrappers.py`, and `docs/pr_handoffs/funding_venue_parser_wrapper_implementation_2026_06_11.md`.
- GitHub PR #192 Files changed incorrectly included the PR #191 planning docs again: `docs/funding_rate_venue_parser_wrapper_plan.md` and `docs/pr_handoffs/funding_venue_parser_wrapper_plan_2026_06_11.md`.
- Those PR #191 planning docs are already merged to main and must not appear in the replacement wrapper implementation PR diff.
- Replacement PR #193 must show exactly three GitHub Files changed: the wrapper module, wrapper tests, and this implementation handoff.
- GitHub PR Files changed is the source of truth and takes precedence over Codex local staged files. If GitHub Files changed differs from Expected Files Changed, merge must remain on hold.
...
- `python -m unittest tests.test_funding_rate_wrappers`: passed; output ended with `Ran 17 tests in 0.019s` and `OK`.
- `python -m unittest discover -s tests`: passed; output ended with `Ran 572 tests in 22.391s` and `OK`.
